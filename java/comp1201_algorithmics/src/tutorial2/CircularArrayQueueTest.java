...
public class CircularArrayQueueTest extends TestCase
{
	...
	public void testMultiInput()
	{
		...
		for( int i = 0; i < 1000; ++i )
		{
			...
			//	int ni2 = queue.noItems();
			//	System.out.println("1 noItems: " + ni2 + "\n");
			
			queue.enqueue( r );
			checkSize( 1, queue );
			
			//	int ni3 = queue.noItems();
			//	System.out.println("2 noItems: " + ni3 + "\n");
	...
	public void testLargeQueue()
	{
		...
		for( int i = 0; i < r.length; ++i )
		{
			assertEquals( "1 Dequeue returns wrong element", r[ i ],
					queue.dequeue() );
		...
		{
			r[ i ] = ( int )Math.round( Math.random() );
			//r[i] = 10 + i; // Predictable test value
			checkSize( i, queue );
			queue.enqueue( r[ i ] );
		}
		for( int i = 0; i < r.length; ++i )
		{
				//System.out.println("before -- r[" + i + "] is: " + r[ i ] + "\n");
				
			assertEquals( "2 Dequeue returns wrong element", r[ i ], queue.dequeue() );
			checkSize( r.length - i - 1, queue );
			...
			/*	try {
				...
				System.out.println("after- - r[" + i + "] is: " + r[ i ] + "\n");
			*/
		}
	}
